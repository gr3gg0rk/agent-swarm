 ...
 */
export async function queryAvailableAgents(mqttClient) {
    ...
    for (const payload of messages) {
        if (payload.length === 0)
            continue; // Skip empty (cleared) messages
        try {
 ...
 */
export async function getAgentById(mqttClient, agentId) {
    if (!mqttClient.getRetainedMessages) {
        throw new Error('MQTT client does not support retained message queries');
    }
    ...
    try {
        const envelope = JSON.parse(messages[0].toString());
        if (envelope.type === 'discovery') {
 ...
 */
export async function getAgentsByRole(mqttClient, role) {
