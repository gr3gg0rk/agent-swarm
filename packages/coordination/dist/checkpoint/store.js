...
export class LocalFileStore {
        ...
        try {
            ...
            if (data.checksum) {
                ...
                if (!validateChecksum(jsonData, data.checksum)) {
                    throw new Error(`CRC32 validation failed: stored=${data.checksum}, computed=${computedChecksum}`);
                }
    ...
    async loadLatest(taskId) {
        try {
            ...
            const taskFiles = files.filter((file) => file.startsWith(`${taskId}-`) && file.endsWith('.json'));
            if (taskFiles.length === 0) {
                return null;
            }
            // Sort by filename timestamp (extract from checkpoint ID or use file stats)
            const fileStats = await Promise.all(taskFiles.map(async (file) => ({
                file,
    ...
    async deleteByTask(taskId) {
        try {
            ...
            await Promise.all(metadata.map((m) => this.delete(m.checkpointId)));
