...
export class TimeoutMonitor {
     ...
     */
    startTimeout(taskId, timeoutMs, retryCount, maxRetries, onTimeout) {
        ...
        const timeoutId = setTimeout(() => {
            ...
            if (retryCount < maxRetries) {
                ...
                console.log(`Task ${taskId} timed out, retrying in ${delay.toFixed(0)}ms (attempt ${retryCount + 1}/${maxRetries})`);
                // Schedule retry
                setTimeout(() => {
 ...
 */
export function calculateBackoffDelay(retryCount, maxDelayMs = 30000, jitterMs = 1000) {
    const baseDelay = Math.pow(2, retryCount) * 1000; // 1s, 2s, 4s, 8s...
 ...
 */
export function createTimeoutMonitor(options) {
