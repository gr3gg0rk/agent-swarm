...
export class TaskCancellation {
     ...
     */
    constructor(mqttClient, taskQueue) {
     ...
     */
    setupCancellationHandler(agentId) {
     ...
     */
    async acknowledgeCancellation(taskId, agentId) {
     ...
     */
    handleCancellationTimeout(taskId) {
        const pending = this.pendingCancellations.get(taskId);
        if (!pending) {
     ...
     * @param reason - Optional cancellation reason
     */
    async publishCancelCommand(agentId, taskId, reason) {
        const payload = {
            taskId,
            reason,
        };
 ...
 */
export function createTaskCancellation(mqttClient, taskQueue) {
