...
export class ArchiveManager {
  ...
  constructor(private db: Database.Database, config: ArchiveConfig = {}) {
    this.config = {
  ...
  archiveOldTasks(): ArchiveResult {
    ...
    const archiveTx = this.db.transaction((cutoff: number) => {
      // Insert into archive
      this.db.prepare(`
    ...
    const pageCountAfter = this.db.pragma('page_count', { simple: true }) as number;
    const pageSizeAfter = this.db.pragma('page_size', { simple: true }) as number;
   ...
   */
  runAllArchives(): { tasks: ArchiveResult; statuses: ArchiveResult } {
    return {
    ...
    this.cronTask = cron.schedule(
      ...
      () => {
        try {
          ...
          console.info(
            `[ArchiveManager] Archived ${results.tasks.archived} tasks, ` +
  ...
  getArchiveStats(): {
    ...
    const tasksResult = tasksStmt.get() as { count: number };
    const statusResult = statusStmt.get() as { count: number };
   ...
   */
  isNearSizeLimit(thresholdMB: number = 40): boolean {
   ...
   */
  archiveIfNeeded(): { tasks: ArchiveResult; statuses: ArchiveResult } | null {
