...
/**
 * Request body for setting context value.
 */
...
export function createContextRoutes(contextStore: ContextStore): Router {
   ...
   */
  router.get('/context/:key', (req: Request, res: Response) => {
    try {
   ...
   * { "value": "1.0.0" }
   */
  router.put('/context/:key', (req: Request, res: Response) => {
    try {
      const { key } = req.params;
      const body = req.body as SetContextBody;
   ...
   */
  router.delete('/context/:key', (req: Request, res: Response) => {
    try {
      const { key } = req.params;
