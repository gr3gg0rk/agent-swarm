...
export class SQLiteSync {
  ...
  private readonly selectByTaskStmt: Database.Statement;
   ...
   */
  private initializeCheckpointsTable(): void {
    this.db.exec(`
  ...
  loadLatest(taskId: string): CheckpointData | null {
    try {
      const result = this.selectByTaskStmt.get(taskId) as { data: string } | undefined;
      ...
      const data = JSON.parse(result.data) as CheckpointData;
      return data;
    } catch (error) {
  ...
  loadCheckpoint(checkpointId: string): CheckpointData | null {
    try {
      const result = this.selectStmt.get(checkpointId) as { data: string } | undefined;
  ...
  listByTask(taskId: string): string[] {
    try {
      const stmt = this.db.prepare(`
        ...
        ORDER BY created_at DESC
      `);
      const results = stmt.all(taskId) as { id: string }[];
   ...
   */
  getCountByTask(): Array<{ taskId: string; count: number }> {
    try {
      const stmt = this.db.prepare(`
        ...
        ORDER BY count DESC
      `);
      return stmt.all() as Array<{ taskId: string; count: number }>;
    } catch (error) {
   ...
   * @returns Number of checkpoints deleted
   */
  deleteOldCheckpoints(taskId: string, keep: number = 5): number {
    try {
      ...
      const keepStmt = this.db.prepare(`
        ...
        LIMIT ?
      `);
      const keepResults = keepStmt.all(taskId, keep) as { id: string }[];
      ...
      // Build params array
      const params: (string | number)[] = [taskId, ...keepIds];
 ...
 */
export function createSQLiteSync(options: SQLiteSyncOptions): SQLiteSync {
