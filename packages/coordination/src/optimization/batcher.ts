...
/**
 * MessageBatcher implements time-windowed batching with dual-trigger flushing.
 *
 * Per 07-RESEARCH.md:
 * - Buffers messages by type and flushes when time threshold expires OR size limit reached
 * - Task assignments bypass batching (latency critical per Open Question 4)
 * - Graceful fallback to direct publish on batcher failure (Pitfall 4)
 *
 * Batch topic pattern: `swarm/batch/{type}` where type is 'tasks', 'status', or 'heartbeats'
 * Batches are MessagePack arrays of existing MessageEnvelope objects for backward compatibility.
 */
/**
 * Buffered message with original topic for routing.
 */
